...
function App() {
  ...
  const initializePWA = () => {
    ...
    if (isPWA) {
      ...
      // Handle login state for PWA
      const token = localStorage.getItem('token');
      const user = JSON.parse(localStorage.getItem('user') || 'null');
      
      // PWA-specific navigation logic
      if (token && user) {
        console.log('PWA: User is logged in');
        // User is logged in, ensure proper routing
        if (window.location.pathname === '/') {
          window.location.replace('/dashboard');
        }
      } else {
        console.log('PWA: User not logged in');
